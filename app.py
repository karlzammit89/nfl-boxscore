...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with _btn_col1:
        _run_recon = st.button("▶️ Run",   use_container_width=True, key="recon_run",
                               disabled=st.session_state.get("recon_running", False))
    with _btn_col2:
                                ...
                                for _dp4 in _dplays:
                                    for _dpt4 in _dp4.get("participants",[]):
                                        ...
                                        if _dl4 and _dl4 not in _dop:
                                            _c2.append({"passer":_dn4,"play_type":(_dp4.get("type",{}).get("text","") or "").lower(),"type_id":(_dp4.get("type",{}).get("id","") or ""),"yds":_dp4.get("statYardage",0),"period":(_dp4.get("period") or {}).get("number","?"),"clock":(_dp4.get("clock") or {}).get("displayValue",""),"text":str(_dp4.get("text",""))[:80]})
